...
def run_heatmap_app():
    ...
    selected_list3 = st.multiselect('군/구별 월 가구소득 데이터 확인하기', column_list[6:])
    ...
    st.subheader('근무시간과 월 가구소득을 라인차트로 비교하기')

    selected_list4 = st.selectbox('근무시간을 선택해주세요.', column_list[:6])
    selected_list44 = st.selectbox('월 가구소득을 선택해주세요.', column_list[6:])
